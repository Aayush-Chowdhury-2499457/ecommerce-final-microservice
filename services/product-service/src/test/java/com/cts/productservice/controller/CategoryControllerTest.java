...
class CategoryControllerTest {
    ...
    void create_invalidBody_returns400() throws Exception {
        mvc.perform(post("/api/categories").header("X-User-Role", "ADMIN")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(CategoryDTO.builder().categoryName("A").build())))
    ...
    void byName_notFound_returns404() throws Exception {
        when(categoryService.findByName("Nope")).thenThrow(new ResourceNotFoundException("Category not found: Nope"));
        mvc.perform(get("/api/categories/categoryName/Nope"))
                .andExpect(status().isNotFound());
    }
    ...
    void update_admin_returns200() throws Exception {
        ...
        mvc.perform(put("/api/categories/1").header("X-User-Role", "ADMIN")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(CategoryDTO.builder().categoryName("Books").build())))
                .andExpect(status().isOk());
    }
    ...
    void update_duplicate_returns409() throws Exception {
        when(categoryService.update(eq(1L), any()))
                .thenThrow(new DuplicateResourceException("Category already exists: Books"));
        mvc.perform(put("/api/categories/1").header("X-User-Role", "ADMIN")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(CategoryDTO.builder().categoryName("Books").build())))
                .andExpect(status().isConflict());
    ...
    @Test
    void delete_hasProducts_returns400() throws Exception {
